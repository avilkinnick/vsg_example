...
int main(int argc, char* argv[])
{
    ...
    while (std::getline(input_file, input_string))
    {
        ...
        else if (input_string == "numverts numfaces")
        {
            ...
            temp_indices = vsg::ushortArray::create(numfaces * 3);
        }
        ...
        else if (input_string == "Texture faces:")
        {
            for (std::size_t i = 0; i < numfaces * 3; ++i)
            {
                unsigned int index;
                input_file >> index;
                current_mesh->indices->at(i) = index - 1;
            }
        }
    }
    ...
    {
        current_mesh->vertices->at(current_mesh->indices->at(i)) = temp_vertices->at(temp_indices->at(i));
    }

    for (std::size_t i = 0; i < numfaces; ++i)
    {
        auto& vertices = current_mesh->vertices;
        auto& indices = current_mesh->indices;
        ...
        auto& normal1 = current_mesh->normals->at(index1);
        auto& normal2 = current_mesh->normals->at(index2);
        auto& normal3 = current_mesh->normals->at(index3);
    ...
    current_mesh->colors = vsg::vec4Array::create(numtverts);
    for (std::size_t i = 0; i < numtverts; ++i)
    {
        current_mesh->colors->at(i).set(1.0f, 1.0f, 1.0f, 1.0f);
    }
    ...
    // Load texture
    vsg::Path texture_path(arguments[2]);
    if (texture_path)
    {
    ...
    mat->value().specular.set(1.0f, 0.9f, 0.5f, 1.0f); // yellow specular highlight
    ...
    graphicsPipelineConfig->assignArray(vertexArrays, "vsg_Color", VK_VERTEX_INPUT_RATE_VERTEX, mesh.colors);
